  ...
  <div>
          ...
          <b-col cols="8" class="d-flex flex-column">
            <div class="section d-flex flex-column h-100">
              <b-row class="m-0">
                <b-col cols="2">
                  <b-form-group>
                    <template slot="label">{{$t('permission-management.permission-control.role')}}
                    </template>
                  ...
                  <div class="text-left">
                    <b-form-group>
                      <b-form-checkbox v-model="isSelectedAllResourcesForRoleForm"
                                       @change="toggleSelectedAllResourcesForRoleForm($event)">
                ...
                <div class="text-left" v-if="selectedRole">
                  <b-form-group>
                    <b-form-checkbox v-model="isSelectedAllResourcesForRole"
                                     @change="toggleSelectedAllResourcesForRole($event)">
                      {{$t('permission-management.permission-control.select-all')}}
                    </b-form-checkbox>
                  </b-form-group>
                </div>
                ...
                <div class="text-right pt-3">
                  <div>
                    <b-button v-if="selectedRole" :disabled="checkPermItem('role_modify')"
                              @click="onClickSaveRole" size="sm" variant="info default"
                              class="mr-3">
                      <i class="icofont-save"/>
                      {{$t('permission-management.permission-control.save')}}
                    </b-button>

                    <b-button @click="onClickDeleteRole(selectedRole)" size="sm"
            ...
            <div class="section d-flex flex-column h-100">
              <b-row class="m-0">
                ...
                <b-col cols="2">
                  <b-form-group>
                    <template slot="label">
                      {{$t('permission-management.permission-control.data-range')}}
                    </template>
                ...
                <b-col cols="8" class="d-flex justify-content-end align-items-center">
                  <div>
                    <b-button size="sm" class="ml-2" variant="info default"
                              @click="searchDataGroup()">
                      <i class="icofont-search-1"/>&nbsp;{{ $t('permission-management.search') }}
                    </b-button>
                    <b-button size="sm" class="ml-2" variant="info default"
                              @click="resetDataGroupSearchForm()">
                      <i class="icofont-ui-reply"/>&nbsp;{{$t('permission-management.reset') }}
                    </b-button>
                    <b-button size="sm" class="ml-2" variant="outline-info default"
                              :disabled="checkPermItem('data_group_export')"
                              @click="onExportButton()">
                      <i class="icofont-share-alt"/>&nbsp;{{ $t('permission-management.export') }}
                    </b-button>
                    <b-button size="sm" class="ml-2" variant="outline-info default"
                              :disabled="checkPermItem('data_group_print')"
                              @click="onPrintGroupButton()">
                      <i class="icofont-printer"/>&nbsp;{{ $t('permission-management.print') }}
                    </b-button>
                    <b-button size="sm" class="ml-2" @click="onClickCreateDataGroup()"
                              :disabled="checkPermItem('data_group_create')"
          ...
          <b-col cols="4" class="pl-0" v-if="selectedDataGroup">
            <div class="section d-flex flex-column h-100 px-3">
              ...
              <div class="text-left">
                <b-form-group>
                  <b-form-checkbox v-model="isSelectedAllUsersForDataGroup"
                                   @change="toggleSelectedAllUsersForDataGroup($event)">
                    {{$t('permission-management.permission-control.select-all')}}
                  </b-form-checkbox>
                </b-form-group>
              </div>
              ...
              <div class="text-right pt-3" v-if="dataGroupDetailStatus==='create'">
                <div>
                  <b-button @click="createDataGroup()" size="sm" variant="info default"><i
                    class="icofont-save"/>
                    {{$t('permission-management.permission-control.save')}}
                  </b-button>
              ...
              <div class="text-right pt-3" v-if="dataGroupDetailStatus!=='create'">
                <div>
                  ...
                  <b-button @click="onClickDeleteDataGroup(selectedDataGroup)" size="sm"
                            variant="danger default"
    ...
    <b-modal centered id="modal-delete-role" ref="modal-delete-role"
      ...
      <template slot="modal-footer">
        <b-button size="sm" variant="primary" @click="deleteRole" class="mr-1">{{$t('system-setting.ok')}}
        </b-button>
        <b-button size="sm" variant="danger" @click="hideModal('modal-delete-role')">
...
<script>
  ...
  import {checkPermissionItem, getDirection, getPermissionInfoId, savePermissionInfo, savePermissionInfoId, getLocale} from "../../../utils";
    ...
    methods: {
            ...
            .then((response) => {
              ...
              switch (message) {
                case responseMessages['ok']:
                  ...
                  //this.$refs.sidebar.checkMenuPermission();
                  //this.roleForm.visible = false;
                  break;
                case responseMessages['used-role-name']:
                  this.$notify('warning', this.$t('permission-management.warning'), this.$t(`response-error-message.used-role-name`), {
        ...
        for (let i = 0; i < data.data.length; i++) {
          ...
          if (resources.length > this.showLength) {
            ...
            temp.resourcesLabel = {
              ...
              label: resources.substr(0, this.showLength) + '...',
              isLong: isLong
            };
          } else {
            temp.resourcesLabel = {
      ...
      fnTransformUserGroupTable(response) {
        ...
        for (let i = 0; i < data.data.length; i++) {
          ...
          let isLong = false;
          if (groupMember.length > this.showLength) {
            isLong = true;
            temp.groupMember = {
              groupMember: groupMember,
              label: groupMember.substr(0, this.showLength) + '...',
              isLong: isLong
            };
          } else {
            temp.groupMember = {
