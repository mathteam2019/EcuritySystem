...
<template>
  <div class="user-management">
      ...
      <b-tab v-if="!checkPermItem('tab_user')" :title="$t('permission-management.member-table')" @click="tabStatus='user'">
                ...
                <b-row>
                  ...
                  <b-col>
                    <b-form-group :label="$t('permission-management.gender')">
                      <b-form-select v-model="filter.gender" :options="genderFilterOptions"
                                     plain/>
                    </b-form-group>
                  </b-col>
                  <b-col>
                  ...
                  >
                    <template slot="userNumber" slot-scope="props">
                                            <span class="cursor-p text-primary"
                                                  @click="onAction('show', props.rowData, props.rowIndex)">{{ props.rowData.userNumber }}</span>
                    </template>
        ...
        <b-row v-show="pageStatus==='create'" class="h-100 form-section">
          <b-col cols="10">
              ...
              <b-col cols="3">
                <b-form-group>
                  <template slot="label">{{$t('permission-management.gender')}}&nbsp;<span
                    class="text-danger">*</span></template>
                  <b-form-select :disabled="profileForm.status==='1000000301'"
            ...
            <b-row class="mb-2">
              <b-col cols="3">
                <b-form-group>
                  <template slot="label">
                    {{$t('permission-management.affiliated-institution')}}&nbsp;<span
                    class="text-danger">*</span></template>
                  <b-form-select :disabled="profileForm.status==='1000000301'"
                                 v-model="profileForm.orgId"
          ...
          <b-col cols="2" class="d-flex align-items-center flex-column">
            ...
            <b-button @click="$refs.profileFile.click()" class="mb-1" variant="info skyblue default"
                      size="sm">{{
        ...
        <b-row v-show="pageStatus==='show'" class="h-100 form-section">
          ...
          <b-col cols="12" class="d-flex justify-content-end align-self-end">
            <b-button :disabled="checkPermItem('user_update_status')"
                      v-if="profileForm.status==='1000000301'"
                      class="mr-1" @click="onAction('inactivate', profileForm)"
                      variant="warning default" size="sm"><i class="icofont-ban"/> {{
              $t('permission-management.action-make-inactive') }}
            </b-button>
            <b-button :disabled="checkPermItem('user_update_status')"
                      v-if="profileForm.status==='1000000302'"
                ...
                <b-col cols="4" class="pr-3">
                  <b-row>
                    ...
                    <b-col>
                      <b-form-group class="search-form-group">
                        <template slot="label">
                          {{$t('permission-management.user.user-group-member')}}
                        </template>
                ...
                <b-col cols="8" class="d-flex justify-content-end align-items-center">
                  <div>
                    <b-button size="sm" class="ml-2" variant="info default"
                              @click="onUserGroupSearchButton()">
                      <i class="icofont-search-1"/>&nbsp;{{ $t('permission-management.search') }}
                    </b-button>
                    <b-button size="sm" class="ml-2" variant="info default"
                              @click="onUserGroupResetButton()">
                      <i class="icofont-ui-reply"/>&nbsp;{{$t('permission-management.reset') }}
                    </b-button>
                    <b-button size="sm" class="ml-2" variant="outline-info default"
                              :disabled="checkPermItem('user_group_export')"
                              @click="onExportButton()">
                      <i class="icofont-share-alt"/>&nbsp;{{ $t('permission-management.export') }}
                    </b-button>
                    <b-button size="sm" class="ml-2" variant="outline-info default"
                              :disabled="checkPermItem('user_group_print')"
                              @click="onPrintGroupButton()">
                      <i class="icofont-printer"/>&nbsp;{{ $t('permission-management.print') }}
                    </b-button>
                    <b-button size="sm" class="ml-2" @click="onUserGroupCreateButton()"
                              :disabled="checkPermItem('user_group_create')"
                              variant="success default">
                      <i class="icofont-plus"/>&nbsp;{{$t('permission-management.new') }}
                    </b-button>
                  </div>
              ...
              <b-row class="flex-grow-1 m-0">
                <b-col cols="12">
                  <div class="table-wrapper table-responsive">
                    ...
                    >
                      <template slot="userGroupNumber" slot-scope="props">
                                                <span class="cursor-p text-primary"
                                                      @click="onUserGroupTableRowClick(props.rowData)">{{ props.rowData.groupNumber }}</span>
                      </template>
                  ...
                  <div class="pagination-wrapper">
                    ...
                    />
                    <b-modal centered ref="modal-prompt-group"
                             :title="$t('permission-management.prompt')">
                      {{$t('permission-management.user.user-group-delete-prompt')}}
                      <template slot="modal-footer">
                        <b-button variant="primary" @click="fnDeleteUserGroupItem()"
                                  class="mr-1">
            ...
            <div class="section d-flex flex-column h-100 px-3">
              ...
              <div class="text-left">
                <b-form-group>
                  <b-form-checkbox v-model="isSelectedAllUsersForDataGroup"
                                   @change="toggleSelectAllUsersForDataGroup($event)">
              ...
              <div class="d-flex align-items-end justify-content-end pt-3"
                ...
                <div>
                  ...
                  <b-button @click="onClickDeleteUserGroup"
                            :disabled="checkPermItem('user_group_delete')"
                            variant="danger default" size="sm"><i
    ...
    <b-modal centered id="model-export" ref="model-export">
      <b-row>
        <b-col cols="12" class="d-flex justify-content-center">
          <h3 class="text-center font-weight-bold" style="margin-bottom: 1rem;">{{
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      onSaveUserPage() {
        ...
        if (this.$v.profileForm.$invalid) {
          ...
          if (this.$v.profileForm.userAccount.$invalid) {
            ...
            } else if (this.profileForm.userAccount.length > 20) {
      ...
      onUserGroupRowClass(dataItem, index) {
