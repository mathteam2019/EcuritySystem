            ...
            <template v-for="(field, fieldIndex) in tableFields">
              <template v-if="field.visible">
                <template v-if="isSpecialField(field.name)">
                  ...
                  >
                    <input type="checkbox"
                      :id="`vuetable-check-body-1-${uuid}-${itemIndex}`"
                      @change="toggleCheckbox(item, field.name, $event)"
                      :checked="rowSelected(item, field.name)">
                    <label :for="`vuetable-check-body-1-${uuid}-${itemIndex}`"></label>
                  </td>
        ...
        <template v-for="(field, fieldIndex) in tableFields">
          <template v-if="field.visible">
            <template v-if="isSpecialField(field.name)">
              ...
              >
                <input type="checkbox"
                  :id="`vuetable-check-body-2-${uuid}-${itemIndex}`"
                  @change="toggleCheckbox(item, field.name, $event)"
                  :checked="rowSelected(item, field.name)">
                <label :for="`vuetable-check-body-2-${uuid}-${itemIndex}`"></label>
              </td>
              <td v-if="extractName(field.name) === '__component'"
                :key="fieldIndex"
                :class="['vuetable-component', field.dataClass]"
